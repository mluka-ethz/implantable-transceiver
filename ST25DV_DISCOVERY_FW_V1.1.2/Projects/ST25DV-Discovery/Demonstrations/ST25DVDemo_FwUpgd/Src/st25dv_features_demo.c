...
static void DisplayEH(uint32_t vdda, uint16_t *eh_values)
{
  ...
  sprintf(msg,"  Voltage  %1.2f  V  ",((float)eh_values[0] * VREF / ADC_RESOLUTION));
  Menu_DisplayString(3, msg);
  /* eh_values[1] is used to store the electrical potential at Rmeas boundaries */
  sprintf(msg,"  Current  %1.1f  mA  ",(((float)eh_values[1]) * VREF / (ADC_RESOLUTION * RMEAS_K)));
  Menu_DisplayString(4, msg);
  sprintf(msg,"  Power   %4.1f  mW  ",(((float)eh_values[1]) * VREF / (ADC_RESOLUTION * RMEAS_K))*((float)eh_values[0] * VREF / ADC_RESOLUTION));
...
//  
//  uint32_t max_length = BSP_NFCTAG_GetByteSize()
...
//    ST25_RETRY(BSP_NFCTAG_GetExtended_Drv()->WriteRFZxSS(ST25DV_PROT_ZONE1, rf_wr_prot));

//    ST25DV_RF_PROT_ZONE rf_rd_prot = {.PasswdCtrl = ST25DV_PROT_PASSWD1 , .RWprotection = ST25DV_READWRITE_PROT};
//    ST25_RETRY(BSP_NFCTAG_GetExtended_Drv()->WriteRFZxSS(ST25DV_PROT_ZONE2, rf_rd_prot));
...
//    BSP_NFCTAG_GetExtended_Drv()->WriteEndZonex(ST25DV_ZONE_END1,(area1_end)/32 + 1);
//   /* area2_starts at next 32bit aligned address */
//   /* Adding +1 here, because tested android smartphones seems to read more bytes than required and thus cross the area border */
//   uint32_t area2_start = ((area1_end/32) + 2)*32;

//    /* Write the area2 CCfile */
//    uint8_t area2_ccfile[4] = {NFCT5_MAGICNUMBER_E1_CCFILE,NFCT5_VERSION_V1_0, (uint8_t)((BSP_NFCTAG_GetByteSize() / 8) - public_size), 0x05};
//    ST25_RETRY(BSP_NFCTAG_WriteData( area2_ccfile, area2_start, 4 ));
//                                                
...
//              
//  /*  BSP_NFCTAG_GetExtended_Drv()->WriteEndZonex(ST25DV_ZONE_END2,0x20);*/
// 
